...
export default function KidsContinentsHub() {
  ...
  return (
<div className="relative min-h-screen w-full overflow-x-hidden overflow-y-auto lg:overflow-hidden flex flex-col items-center select-none transition-colors duration-500" dir="rtl">
      
      {/* 1. الخلفية المنقطة الثابتة */}
      <div className="fixed inset-0 z-0 pointer-events-none bg-slate-50 dark:bg-black">
        {/* طبقة النقاط - CurrentColor تجعلها تتوافق مع وضع النهار والليل */}
        <div 
        ...
        />
        {/* تدرج لوني خفيف ليعطي روحاً للخلفية */}
        <div className="absolute inset-0 bg-gradient-to-b from-sky-100/40 via-transparent to-white/10 dark:from-sky-900/20 dark:to-transparent" />
      </div>

      {/* 2. حاوية المحتوى الرئيسية - شفافة لتظهر الخلفية */}
      <div className="relative z-10 w-full flex flex-col items-center bg-transparent">
        
        {/* الهيدر: أزرار التحكم */}
        <div className="relative w-full max-w-7xl flex justify-between items-center py-8 px-6 z-30">
          <Link href="/">
        ...
        {/* 3. عرض القارات (Desktop) */}
        <div className="hidden lg:block relative flex-1 w-full max-w-[1400px] z-20 min-h-[85vh]">
                ...
                <div className="relative flex flex-col items-center pointer-events-none">
                  ...
                  <span 
                    className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white border-[4px] border-black px-8 py-3 rounded-2xl text-black font-black text-3xl transition-all duration-300 shadow-[8px_8px_0_0_#000] pointer-events-none whitespace-nowrap z-40
                      ${isHovered ? "opacity-100 scale-100 -translate-y-[130%]" : "opacity-0 scale-50"}
                    `}
                  >
                    {continent.name}
                  </span>
              ...
              <div className="w-full md:w-1/2 p-10 flex flex-col justify-center text-right">
                ...
                <div className="flex flex-col sm:flex-row gap-6">
                  <Link href={`/kids/continent/${selectedContinent.id}`} className="flex-1">
                    <button className="w-full bg-sky-400 border-[5px] border-black text-black py-5 rounded-[1.5rem] font-black text-3xl shadow-[8px_8px_0_0_#000] hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all">
                      استكشاف
                    </button>
                  </Link>
